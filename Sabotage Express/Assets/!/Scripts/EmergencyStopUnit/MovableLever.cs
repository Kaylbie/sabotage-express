...
public class MovableLever : Interactable
{
    private Vector3 initialPosition;
    private bool isConnected;
    private const string portTag = "Port";
    [SerializeField] private float dragResponseThreshold = 0.1f;
    ...
    public void Update()
    {
        if (isInUse)
        {
            ...
            else
            {
                ...
                if (!isConnected)
                {
                    transform.position = new Vector3(transform.position.x, newWorldPosition.y, transform.position.z);
                }
